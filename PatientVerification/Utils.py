# From: https://github.com/SANCHES-Pedro/privacy_diffusion_medical/blob/557d7372992e8f13a8f564b5739ade53a7d7c551/eval/quantitative/PatientVerification/utils/Utils.py
...
# @Virginia added
def get_data_loaders_MONAI(phase = 'training', batch_size = 16, shuffle = True, num_workers = 16, pin_memory = True,
                           local = False):

    if phase == 'testing':
        folder_files = './image_pairs/pairs_testing.txt'
    elif phase == 'validation':
        folder_files = './image_pairs/pairs_validation.txt'
    else:
        folder_files = './image_pairs/pairs_training.txt'

    dicts = getDicts(folder_files, local = local)
    dataset = getDataset(dicts, is_train=phase=='training',)
    return getLoader(dataset, batch_size=batch_size, shuffle=shuffle, num_workers=num_workers,
                     pin_memory = pin_memory)
...
def train(net, training_loader, n_samples, batch_size, criterion, optimizer, epoch, n_epochs,
          summary_writer, interval_writer = 300):
    ...
    for i, batch in enumerate(training_loader):
        ...
        with torch.no_grad():
            outputspos = outputs[0:outputs.shape[0]:2, ...]
            outputsneg = outputs[1:outputs.shape[0]:2, ...]
            labelsneg = labels[1:labels.shape[0]:2, ...]
            labelspos = labels[0:labels.shape[0]:2, ...]
            labelspos = labelspos.type_as(outputspos)
            labelsneg = labelsneg.type_as(outputsneg)
            loss_positives = criterion(outputspos, labelspos).item()
            loss_negatives = criterion(outputsneg, labelsneg).item()
            print("Loss for positives %.3f" %loss_positives)
            print("Loss for negatives %.3f" %loss_negatives)
            if len(torch.unique(net.fc_end.weight.grad)) == 1:
                print("Vanishing gradient")

        running_loss += loss.item()
        accuracy = metrics.accuracy_score(y_pred= torch.sigmoid(outputs).detach().cpu().round(),
                                          y_true=labels.detach().cpu())

        if i%interval_writer == 0:
            summary_writer.add_scalar("total_loss", loss.item(), epoch*n_epochs+i)
            summary_writer.add_scalar("loss_positives", loss_positives, epoch * n_epochs + i)
            summary_writer.add_scalar("loss_negatives", loss_negatives, epoch * n_epochs + i)
            summary_writer.add_scalar("accuracy", accuracy.item(), epoch * n_epochs + i)

        print('Epoch [%d/%d], Iteration [%d/%d], Loss: %.4f, Acc. %.4f' % (
            epoch + 1,
            n_epochs,
            i + 1,
...
def bootstrap(n_bootstraps, y_true, y_pred, path=None, experiment_description = None, ):
    y_true = np.array(y_true)
    y_pred = np.array(y_pred)

    bootstrapped_scores = []

    if not (path is None or experiment_description is None):
